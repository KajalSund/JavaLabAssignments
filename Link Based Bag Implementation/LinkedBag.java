...
// or null otherwise.
    ...
    public T remove() {
        ...
        {
        	p=(int)(Math.random()*50+1);//to check if the random number is in the range
        }
        int i=1;
        Node currentNode=firstNode;
        for(;i<p&&currentNode!=null;i++)
        {
        	currentNode=currentNode.next;//currentNode will point to the random node
        }
    ...
    public void removeDuplicates() {
    	...
    	if(numberOfEntries<=1)
    		return;
    	LinkedBag<T>tempbag=new LinkedBag<>();//tempbag stores unique elements
    	tempbag.add(firstNode.data);
    	Node currnode;
    	
    	while(!isEmpty())
    	{//remove duplicate present of the current node
    		remove(firstNode.data);//one by one remove node from this bag
    		currnode=firstNode;
    		while(currnode!=null)
    		{
